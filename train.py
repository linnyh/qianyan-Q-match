...
def do_train():
    ...
    # 数据集准备
    train_ds = load_dataset(  # paddlenlp 内置函数
    ...
    model = paddle.DataParallel(model)  # 通过数据并行模式执行动态图模型

    # 训练步数 = 实例数量 * 训练世代数
    num_training_steps = len(train_data_loader) * args.epochs

    # 线性衰减的学习率，前10%steps按比率从 0 线性增加到learning_rate 后缓慢线性衰减
    ...
    optimizer = paddle.optimizer.AdamW(
        ...
        weight_decay=args.weight_decay,  # 衰减权重
